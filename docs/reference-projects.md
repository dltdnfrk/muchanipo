...
릴리스 표현은 이 문서의 서술이 아니라 `muchanipo references --json`의 `ready`, `gaps`, `not_ready_references`, `license_warnings` 값을 기준으로 해야 한다. `ready=false`인 항목은 제품 동작 목표 또는 부분 포트로만 설명하고, 완전 이식/99% 구현/상용 준비 같은 문구에 포함하면 안 된다.
...
Muchanipo에서는 다중 에이전트 연구 흐름의 실제 통합 대상이다. 질문을 여러 방향으로 쪼개는 방식, 근거를 찾은 뒤 보고서를 쓰는 방식, 인물·조직·이해관계자 기반 페르소나를 만드는 방식, 관계 정보를 고려한 토론 구조를 제품 동작으로 가져와야 한다. 현재 Council 런타임은 온톨로지 엔티티에서 MiroFish식 프로필을 만들고, depth별로 수십~수백 개 persona pool을 생성한 뒤, 라운드별 active speaker를 순차 호출하며 transcript를 남긴다. AGPL-3.0 라이선스 경계 때문에 upstream 전체 vendoring은 별도 compliance review가 필요하지만, 제품 표면에서는 문서상 참고가 아니라 실제 포트/어댑터 동작으로 취급한다.
