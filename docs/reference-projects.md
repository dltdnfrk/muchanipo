...
이 표는 구현 우선순위를 정하는 용도가 아니라, 각 단계에서 어떤 외부 프로젝트 로직이 실제 제품 동작으로 들어가야 하는지 확인하는 색인이다. 세부 설명은 아래 항목별 정리를 기준으로 한다.
...
- 실제 플러그인 출처: https://github.com/fivetaku/show-me-the-prd
...
Plannotator는 사람이 중간에 검토하는 절차의 실제 vendored reference다. 원본 소스는
`third_party/plannotator`에 upstream commit
...
`src/pipeline/idea_to_council.py`가 targeting 전에 해당 변경을 실제
`ResearchBrief`, `planning_prd`, `feature_hierarchy`, `user_flow`에 적용한다.
적용 가능한 annotation은 `type=edit`, 신뢰된 Plannotator source, 명시적
replacement/value/text, 그리고 HITL payload에 공개된 target allowlist를 모두
만족해야 한다. 알 수 없는 annotation target, comment-only annotation,
...
운영 가드레일: 로컬 seed가 현재 농업 기술용 부분 집합이라는 사실과 Nemotron-Personas-Korea 전체의 용도를 혼동하면 안 된다. 농업·AgTech 주제가 아닐 때, 특히 CLI/TUI/runtime 코드리뷰에서는 `agtech_farmer_seed`를 그대로 쓰지 않는다. 이 경우에는 Nemotron-Personas-Korea의 지역·직업·인구통계 스키마를 grounding 형식으로 참고하되, 실제 역할 ontology는 코드리뷰 대상에 맞춰 Python CLI maintainer, terminal/TUI operator, runtime/SRE, QA, privacy/security/license reviewer 같은 개발·운영 역할에서 뽑아야 한다. 데이터셋 기록을 직접 쓰지 않은 경우에는 산출물에 "schema-grounded, not dataset-sampled"처럼 출처 경계를 명시한다.

## HACHIMI 페르소나 생성

- 출처: https://github.com/ZeroLoss-Lab/HACHIMI
- 논문: arXiv:2603.04855
- 라이선스: MIT
...
- 출처: https://github.com/EvoAgentX/EvoAgentX
- 라이선스: MIT
