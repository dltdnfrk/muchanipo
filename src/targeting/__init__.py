...
Every institution, journal, and seed paper MUST be backed by an academic
API response. LLM generation is strictly prohibited for these fields.
"""
...
class TargetingMap:
    ...
    Fields follow PRD-v2 §3.5 schema exactly. The optional ``provenance``
    dict tracks API response references so that ``validate_targeting_map``
    in ``safety.lockdown`` can enforce the hallucination-prevention rule.
    """
